...
import { TpJeeTestModule } from '../../../test.module';
  ...
  describe('Alert Error Component', () => {
    ...
    beforeEach(async(() => {
      TestBed.configureTestingModule({
        imports: [TpJeeTestModule, TranslateModule.forRoot()],
    ...
    describe('Error Handling', () => {
      it('Should display an alert on status 0', () => {
        // GIVEN
        eventManager.broadcast({ name: 'tpJeeApp.httpError', content: { status: 0 } });
        // THEN
      ...
      it('Should display an alert on status 404', () => {
        // GIVEN
        eventManager.broadcast({ name: 'tpJeeApp.httpError', content: { status: 404 } });
        // THEN
        expect(comp.alerts.length).toBe(1);
      ...
      it('Should display an alert on generic error', () => {
        // GIVEN
        eventManager.broadcast({ name: 'tpJeeApp.httpError', content: { error: { message: 'Error Message' } } });
        eventManager.broadcast({ name: 'tpJeeApp.httpError', content: { error: 'Second Error Message' } });
        // THEN
      ...
      it('Should display an alert on status 400 for generic error', () => {
        ...
        eventManager.broadcast({ name: 'tpJeeApp.httpError', content: response });
        // THEN
        expect(comp.alerts.length).toBe(1);
